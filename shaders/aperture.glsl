...
#pragma parameter SCANLINE_SHAPE "Scanline Shape" 1.5 1.0 100.0 0.1
...
vec3 filter_lanczos(sampler2D tex, vec2 co, vec2 tex_size, float sharp)
{
    ...
    vec2 tex_co = (floor(pix_co) + vec2(0.5)) / tex_size;
...
{
    float i = mod(floor(x * OUTPUT_SIZE.x / MASK_SIZE), MASK_COLORS);
...
void main()
{
    float scale = floor((OUTPUT_SIZE.y / INPUT_SIZE_0.y) + 0.001);
    float offset = 1.0 / scale * 0.5;

    if (bool(mod(scale, 2.0))) offset = 0.0;

    vec2 co = (v_texCoord * INPUT_SIZE_0 - vec2(0.0, offset * SCANLINE_OFFSET)) / INPUT_SIZE_0;

    vec3 col_glow = filter_gaussian(INPUT_TEXTURE_0, co, INPUT_SIZE_0);
    vec3 col_soft = filter_lanczos(INPUT_TEXTURE_0, co, INPUT_SIZE_0, SHARPNESS_IMAGE);
    vec3 col_sharp = filter_lanczos(INPUT_TEXTURE_0, co, INPUT_SIZE_0, SHARPNESS_EDGES);
    vec3 col = sqrt(col_sharp * col_soft);

    col *= get_scanline_weight(fract(co.y * INPUT_SIZE_0.y), col_soft);
