...
#pragma parameter MASK "Mask Type" 3.0 0.0 3.0 1.0
#pragma parameter MASK_INTENSITY "Mask Intensity" 0.44 0.0 1.0 0.05
#pragma parameter SCANLINE_THINNESS "Scanline Intensity" 0.00 0.0 1.0 0.1
#pragma parameter SCAN_BLUR "Sharpness" 8.8 1.0 3.0 0.1
...
#pragma parameter CORNER "Corner Round" 2.5 0.0 11.0 1.0
...
//--------------------------------------------------------------
#define INPUT_X INPUT_SIZE_0.x
#define INPUT_Y INPUT_SIZE_0.y
//--------------------------------------------------------------
...
// uv+=vec2(0.5,0.5);
 ...
 return FromSrgb(texture(INPUT_TEXTURE_0,uv.xy,-16.0).rgb);}
#endif
...
//--------------------------------------------------------------
    ...
    CrtsF2 ipos,
//--------------------------------------------------------------
    // INPUT_SIZE_0 / OUTPUT_SIZE (in pixels)
    CrtsF2 inputSizeDivOutputSize,
//--------------------------------------------------------------
    // 0.5 * INPUT_SIZE_0 (in pixels)
    CrtsF2 halfInputSize,
//--------------------------------------------------------------
    // 1.0 / INPUT_SIZE_0 (in pixels)
    CrtsF2 rcpInputSize,
//--------------------------------------------------------------
    // 1.0 / OUTPUT_SIZE (in pixels)
    CrtsF2 rcpOutputSize,
//--------------------------------------------------------------
    // 2.0 / OUTPUT_SIZE (in pixels)
...
void main()
{
    ...
    FragColor.rgb = CrtsFilter(v_texCoord * OUTPUT_SIZE,
    INPUT_SIZE_0 / OUTPUT_SIZE,
    INPUT_SIZE_0 * vec2(0.5,0.5),
    1.0/INPUT_SIZE_0,
    1.0/OUTPUT_SIZE,
    2.0/OUTPUT_SIZE,
    INPUT_SIZE_0.y,
  ...
  #ifdef SHOW_DRY
    if (v_texCoord.x > SHOW_DRY)
     if (SHOW_DRY_MODE == 0)
        FragColor = vec4(texture(INPUT_TEXTURE_0, v_texCoord).rgb, 1.0);
     else
